...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating Iebs Identities"
    ...
    infoln "Creating Cantabria Identities"
  ...
  if [ "$CRYPTO" == "cfssl" ]; then
    ...
    peer_cert peer peer0.iebs.universidades.com iebs
    peer_cert admin Admin@iebs.universidades.com iebs

    infoln "Creating Cantabria Identities"
    #function_name cert-type   CN   org
    peer_cert peer peer0.cantabria.universidades.com cantabria
    peer_cert admin Admin@cantabria.universidades.com cantabria

    infoln "Creating Orderer Org Identities"
    #function_name cert-type   CN   
    orderer_cert orderer orderer.universidades.com
    orderer_cert admin Admin@universidades.com
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    while :
    do
      if [ ! -f "organizations/fabric-ca/iebs/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
  ...
  infoln "Generating CCP files for Iebs and Cantabria"
...
# call the script to create the channel, join the peers of iebs and cantabria,
...
function networkDown() {
  ...
  # stop org3 containers also in addition to iebs and cantabria, in case we were running sample to add org3
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} volume rm docker_orderer.universidades.com docker_peer0.iebs.universidades.com docker_peer0.cantabria.universidades.com
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/iebs/msp organizations/fabric-ca/iebs/tls-cert.pem organizations/fabric-ca/iebs/ca-cert.pem organizations/fabric-ca/iebs/IssuerPublicKey organizations/fabric-ca/iebs/IssuerRevocationPublicKey organizations/fabric-ca/iebs/fabric-ca-server.db'
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/cantabria/msp organizations/fabric-ca/cantabria/tls-cert.pem organizations/fabric-ca/cantabria/ca-cert.pem organizations/fabric-ca/cantabria/IssuerPublicKey organizations/fabric-ca/cantabria/IssuerRevocationPublicKey organizations/fabric-ca/cantabria/fabric-ca-server.db'
