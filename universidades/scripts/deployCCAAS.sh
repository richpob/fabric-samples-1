...
startDockerContainer() {
  ...
  if [ "$CCAAS_DOCKER_RUN" = "true" ]; then
    ...
    set -x
    ${CONTAINER_CLI} run --rm -d --name peer0iebs_${CC_NAME}_ccaas  \
                  --network universidades_network \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest

    ${CONTAINER_CLI} run  --rm -d --name peer0cantabria_${CC_NAME}_ccaas \
                  --network universidades_network \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest
    ...
    infoln "    ${CONTAINER_CLI} run --rm -d --name peer0iebs_${CC_NAME}_ccaas  \
                  --network universidades_network \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest"
    infoln "    ${CONTAINER_CLI} run --rm -d --name peer0cantabria_${CC_NAME}_ccaas  \
                  --network universidades_network \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest"
...
## Install chaincode on peer0.iebs and peer0.cantabria
infoln "Installing chaincode on peer0.iebs..."
installChaincode 1
infoln "Install chaincode on peer0.cantabria..."
installChaincode 2
...
## expect iebs to have approved and cantabria not to
checkCommitReadiness 1 "\"IebsMSP\": true" "\"CantabriaMSP\": false"
checkCommitReadiness 2 "\"IebsMSP\": true" "\"CantabriaMSP\": false"

## now approve also for cantabria
approveForMyOrg 2

## check whether the chaincode definition is ready to be committed
## expect them both to have approved
checkCommitReadiness 1 "\"IebsMSP\": true" "\"CantabriaMSP\": true"
checkCommitReadiness 2 "\"IebsMSP\": true" "\"CantabriaMSP\": true"
