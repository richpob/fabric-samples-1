...
function peer_cert() {
    ...
    mkdir -p "organizations/peerOrganizations/$ORG.universidades.com/ca"
    mkdir -p "organizations/peerOrganizations/$ORG.universidades.com/msp/cacerts"
    mkdir -p "organizations/peerOrganizations/$ORG.universidades.com/msp/tlscacerts"
    mkdir -p "organizations/peerOrganizations/$ORG.universidades.com/peers"
    mkdir -p "organizations/peerOrganizations/$ORG.universidades.com/tlsca"

    CERT_DIR=organizations/peerOrganizations/$ORG.universidades.com

    if [ ! -f "$CERT_DIR/ca/ca-key.pem" ]; then

        cfssl gencert -initca "${PWD}/organizations/cfssl/ca-peer.json" | cfssljson -bare "$CERT_DIR/ca/ca"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/tlsca/tlsca.$ORG.universidades.com-cert.pem"
        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/ca/ca.$ORG.universidades.com-cert.pem"
...
function orderer_cert() {
    TYPE=$1 #orderer user
    USER=$2 #orderer.universidades.com

    mkdir -p organizations/ordererOrganizations/universidades.com/ca
    mkdir -p organizations/ordererOrganizations/universidades.com/msp/cacerts
    mkdir -p organizations/ordererOrganizations/universidades.com/msp/tlscacerts
    mkdir -p organizations/ordererOrganizations/universidades.com/orderers
    mkdir -p organizations/ordererOrganizations/universidades.com/tlsca

    CERT_DIR=organizations/ordererOrganizations/universidades.com

    if [ ! -f "$CERT_DIR/ca/ca-key.pem" ]; then

        cfssl gencert -initca "${PWD}/organizations/cfssl/ca-orderer.json" | cfssljson -bare "$CERT_DIR/ca/ca"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/tlsca/tlsca.universidades.com-cert.pem"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/msp/cacerts/"
        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/msp/tlscacerts/tlsca.universidades.com-cert.pem"
...
function generate_orderer_certs() {
    ...
    for DIR in cacerts keystore signcerts tlscacerts; do
        mkdir -p "organizations/ordererOrganizations/universidades.com/orderers/$USER/msp/$DIR"
    done

    mkdir -p "organizations/ordererOrganizations/universidades.com/orderers/$USER/tls"
    ...
    cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/orderers/$USER/msp/tlscacerts/tlsca.universidades.com-cert.pem"
